...
PBAS::PBAS(void)
    : N(20), R_lower(18), Raute_min(2), T_lower(2), T_upper(200), R_scale(5),
...
bool PBAS::process(cv::Mat *input, cv::Mat *output) {
    if (width != input->cols) {
        ...
        if (input->rows < 1 || input->cols < 1) {
            std::cout << "Error: Occurrence of to small (or empty?) image size "
                         "in PBAS. STOPPING "
                      << std::endl;
            return false;
        }
    ...
    for (int j = 0; j < resultMap->rows; ++j) {
        ...
        for (int z = 0; z < chans; ++z) {
            ...
            currentFeaturesC_Pt.push_back(
                currentFeatures.at(z + chans).ptr<uchar>(j));
        ...
        for (int k = 0; k < runs; ++k) {
            for (int z = 0; z < chans; ++z) {
                B_Mag_Pts.at(z).push_back(
                    backgroundModel.at(k).at(z).ptr<float>(j));
                B_Col_Pts.at(z).push_back(
                    backgroundModel.at(k).at(z + chans).ptr<uchar>(j));
            }
        }

        for (int i = 0; i < resultMap->cols; ++i) {
            // Compare each pixel to in the worst runtime-case each background
            // model
            int count = 0;
            int index = 0;
            ...
            do {
                if (chans == 3) {
                    norm = sqrt((((double)B_Mag_Pts.at(0).at(index)[i] -
                                  ((double)*currentFeaturesM_Pt.at(0))) *
                                 ((double)B_Mag_Pts.at(0).at(index)[i] -
                                  ((double)*currentFeaturesM_Pt.at(0)))) +
                                (((double)B_Mag_Pts.at(1).at(index)[i] -
                                  ((double)*currentFeaturesM_Pt.at(1))) *
                                 ((double)B_Mag_Pts.at(1).at(index)[i] -
                                  ((double)*currentFeaturesM_Pt.at(1)))) +
                                (((double)B_Mag_Pts.at(2).at(index)[i] -
                                  ((double)*currentFeaturesM_Pt.at(2))) *
                                 ((double)B_Mag_Pts.at(2).at(index)[i] -
                                  ((double)*currentFeaturesM_Pt.at(2)))));

                    dist = sqrt((((double)B_Col_Pts.at(0).at(index)[i] -
                                  ((double)*currentFeaturesC_Pt.at(0))) *
                                 ((double)B_Col_Pts.at(0).at(index)[i] -
                                  ((double)*currentFeaturesC_Pt.at(0)))) +
                                (((double)B_Col_Pts.at(1).at(index)[i] -
                                  ((double)*currentFeaturesC_Pt.at(1))) *
                                 ((double)B_Col_Pts.at(1).at(index)[i] -
                                  ((double)*currentFeaturesC_Pt.at(1)))) +
                                (((double)B_Col_Pts.at(2).at(index)[i] -
                                  ((double)*currentFeaturesC_Pt.at(2))) *
                                 ((double)B_Col_Pts.at(2).at(index)[i] -
                                  ((double)*currentFeaturesC_Pt.at(2)))));
                } else {
            ...
            if (count >= Raute_min) {
                *resultMap_Pt = 0;
                double ratio =
                    std::ceil((double)T_upper / (double)(*actualT_Pt));
                // in the first run every distance is zero, because there is no
                // background model  in the secont run, we have already one
                // image as background model, hence a reasonable minDist could
                // be found -> because of the partly 1/run changing in the
                // running average, we set in the first try meanMinDist to the
                // actual minDist value
                if (runs < N && runs > 2) {
                    *meanMinDist_Pt =
                        ((((float)(runs - 1)) * (*meanMinDist_Pt)) +
                         (float)minDist) /
                        ((float)runs);
                ...
                if (runs == N) {
                    ...
                    if (randomT.at(entry) < ratio) {
                        ...
                        int rand = randomN.at(
                            entry +
                            1); // randomGenerator.uniform((int)0,(int)N-1);
                        for (int z = 0; z < chans; ++z) {
                            B_Mag_Pts.at(z).at(rand)[i] =
                                (float)*currentFeaturesM_Pt.at(z);
                            B_Col_Pts.at(z).at(rand)[i] =
                                (uchar)*currentFeaturesC_Pt.at(z);
                        }

                        *meanMinDist_Pt =
                            ((((float)(N - 1)) * (*meanMinDist_Pt)) +
                             (float)minDist) /
                            ((float)N);
                    }
                    ...
                    if (randomTN.at(entry) < ratio) {
                        ...
                        for (int z = 0; z < chans; ++z) {
                            (backgroundModel.at(rand))
                                .at(z)
                                .at<float>(yNeigh, xNeigh) =
                                currentFeatures.at(z).at<float>(yNeigh, xNeigh);
                            (backgroundModel.at(rand))
                                .at(z + chans)
                                .at<uchar>(yNeigh, xNeigh) =
                                currentFeatures.at(z + chans).at<uchar>(yNeigh,
                                                                        xNeigh);
                        }
                    }
                }
    ...
    // cv::Mat R = actualR.clone();
    // cv::Mat T = actualT.clone();
    ...
    //        cv::imshow("test split color (" + std::to_string(i) + ")",
    //        backgroundModel[i][1]);
    ...
    double meanMag =
        sumMagnitude / (double)(glCounterFore + 1); // height*width);
...
void PBAS::calculateFeatures(std::vector<cv::Mat> *feature,
                             cv::Mat *inputImage) {
    if (!feature->empty())
        feature->clear();
...
void PBAS::setRaute_min(int temp) { Raute_min = temp; }

void PBAS::setR_lower(double temp) { R_lower = temp; }

void PBAS::setR_incdec(double temp) { R_incdec = temp; }

void PBAS::setR_scale(double temp) { R_scale = temp; }

void PBAS::setT_init(double temp) { T_init = temp; }

void PBAS::setT_lower(double temp) { T_lower = temp; }
...
void PBAS::setT_dec(double temp) { T_dec = temp; }

void PBAS::setT_inc(double temp) { T_inc = temp; }

void PBAS::setAlpha(double temp) { alpha = temp; }

void PBAS::setBeta(double temp) { beta = temp; }

bool PBAS::isMovement() { return isMove; }
