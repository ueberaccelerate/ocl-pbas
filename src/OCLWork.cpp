...
OCLWork::OCLWork() {
  ...
  err = clGetDeviceIDs(_platform_ids[1], CL_DEVICE_TYPE_CPU, 1, &_device_ids,
                       nullptr);
  cl_int uerr;
  ...
  if (uerr != CL_SUCCESS) {
    ...
    errcode = clGetProgramBuildInfo(_program, _device_ids, CL_PROGRAM_BUILD_LOG,
...
void OCLWork::enqueue_image_write(cl_mem &mem, cl_uint width, cl_uint height,
  ...
  cl_int err;
  err = clEnqueueWriteImage(_command_queue, mem, CL_TRUE, origin, region, 0, 0,
                            data, 0, NULL, NULL);
  MCLASSERT(err);
}
...
void OCLWork::get_platform_all_info() {
  ...
  cl_platform_info list_info[num_info] = {CL_PLATFORM_VERSION, CL_PLATFORM_NAME,
                                          CL_PLATFORM_VENDOR,
  ...
  for (size_t i = 0; i < _count_platform; i++) {
    ...
    err =
        clGetPlatformInfo(_platform_ids[i], CL_PLATFORM_NAME, size, name, NULL);
...
void OCLWork::get_device_info() {
  ...
  err =
      clGetDeviceInfo(_device_ids, CL_DEVICE_OPENCL_C_VERSION, 0, NULL, &size);
