...
MPBAS::PBASImpl::~PBASImpl() {
  ...
  std::for_each(m_cl_mem_D, m_cl_mem_D + N,
                [](const auto mem) { clReleaseMemObject(mem); });
  std::for_each(m_cl_mem_M, m_cl_mem_M + N,
                [](const auto mem) { clReleaseMemObject(mem); });
    ...
    if (I.channels() == 3) {
      for (size_t i = 0; i < _index; i++) {
        for (size_t j = 0; j < 6; j++) {
          cv::imshow("test split color (" + std::to_string(j) + ")",
                     _model[i * 6 + j]);
        }
        cv::waitKey(30);
      }
    ...
    for (size_t y = 0; y < src.rows; y++) {
      for (size_t x = 0; x < src.cols; x++) {
        ...
        if (index_r >= min) {
          ...
          if (_index == N) {
            double ratio =
                std::ceil((double)T_upper / (double)_T.at<float>(y, x));
            ...
            if (randomTN.at(entry) < ratio) {
              ...
              int n_y = -1 + rand() % 3;
              int n_x = -1 + rand() % 3;
              if (y + n_y > 0 && y + n_y < src.rows && x + n_x > 0 &&
                  x + n_x < src.cols) {
                _model[rand_ind * 2 + 0].at<uchar>(y + n_y, x + n_x) =
                    feature[0].at<uchar>(y + n_y, x + n_x);
                _model[rand_ind * 2 + 1].at<float>(y + n_y, x + n_x) =
...
//}
    ...
    if (m_cl_index < N) {
      if (m_cl_index == 0) {
        ...
        set_arg_fill_R_T_kernel(m_cl_fill_R_T_kernel, m_cl_mem_T, WIDTH, HEIGHT,
                                m_cl_mem_R, T_lower, R_lower);
        m_work.run_cl_kernel(m_cl_fill_R_T_kernel, cv::Vec2i(WIDTH, HEIGHT),
                             cv::Vec2i(16, 16), e);
      }
      ...
      // cl_float fill = 0.f;
      // m_work.enqueue_buffer_fill(m_cl_mem_D[m_cl_index], sizeof(cl_float) *
    ...
    m_work.enqueue_buffer_read(
        m_cl_mem_feature, sizeof(cl_float2) * WIDTH * HEIGHT, mat_M.data, e);
    // m_work.enqueue_buffer_read(m_cl_mem_D[m_cl_index-1],
    // sizeof(cl_float)*WIDTH*HEIGHT, D.data, e);
...
// cv::waitKey(0);
    ...
    m_work.enqueue_buffer_fill(m_cl_mem_avrg_Im, sizeof(cl_float), &average_mag,
                               sizeof(cl_float), e);
    ...
    while (index_l < m_cl_index) {
      ...
      m_work.run_cl_kernel(m_cl_pbas_part1_kernel, cv::Vec2i(WIDTH, HEIGHT),
                           cv::Vec2i(16, 16), e);
    ...
    m_work.enqueue_buffer_read(m_cl_mem_T, sizeof(cl_float) * WIDTH * HEIGHT,
                               T.data, e);
...
// m_work.enqueue_buffer_read(m_cl_mem_avrg_Im, sizeof(cl_float), &m_cl_avrg_Im,
// e);  m_cl_avrg_Im /= WIDTH*HEIGHT;  printf("\tavrg: %f\r", m_cl_avrg_Im);
...
void MPBAS::PBASImpl::create_images() {
  ...
  return;
  for (size_t i = 0; i < N; i++) {
    //         m_work.create_image(m_cl_MI[i], CL_MEM_READ_WRITE, WIDTH, HEIGHT,
