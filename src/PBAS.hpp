...
//
// Note: some changes, to improve the speed and memory requirements, were
// achieved in comparison to the  described PBAS algorithm in the paper above.
//
// Example usage:
...
class PBAS {
  ...
  private:
    ...
    std::vector<int> randomN, randomMinDist, randomX, randomY, randomT,
        randomTN;
    ...
    // factor which defines new threshold of R(x_i) together with
    // meanMinDist(x_i)
    // scale for the sphere threshhold to define pixel-based Thresholds
    double R_scale;

    // decreasing / increasing factor of R(x_i)
    // increasing/decreasing factor for the r-Threshold based on the result of
    // rTreshScale * meanMinDistBackground
    double R_incdec;
    ...
    // Defining the number of background-model-images, which have a
    // lowerDIstance to the current Image than defined by the R-Thresholds, that
    // are necessary to decide that this pixel is background
    int Raute_min;
    //#####################################################################################
    //####################################################################################
    ...
    // scale that defines the increasing of the update rate of the background
    // model, if the current pixel is background
    //--> more frequently updates if pixel is background because, there shouln't
    // be any change
    double T_inc;

    // upper boundary of T(x_i)
    // defining an upper value, that nrSubsampling can achieve, thus it doesn't
    // reach to an infinite value, where actually no update is possible at all
    double T_upper;
    ...
    //--> Thesis: Our Foreground is a real moving object -> thus the
    // background-model is good, so don't update it
    double T_dec;
