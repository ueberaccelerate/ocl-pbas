...
kernel void fill_T_R(global float *oT, global float *oR, const uint width,
                     const uint height, const uint T_lower,
                     const uint R_lower) {
...
kernel void pbas_part2(
  ...
  global float2 *mi =
      choose_m(&rand_n[jj * width + ii], model_size, M0, M1, M2, M3, M4, M5, M6,
               M7, M8, M9, M10, M11, M12, M13, M14, M15, M16, M17, M18, M19);
    ...
    if (index_l == model_size) {
      ...
      if (rand_T < ratio) {
        if (IS_BOUNDS(ii * jj, width * height)) {
          ...
          avr = D0[pos_b] + D1[pos_b] + D2[pos_b] + D3[pos_b] + D4[pos_b] +
                D5[pos_b] + D6[pos_b] + D7[pos_b] + D8[pos_b] + D9[pos_b] +
                D10[pos_b] + D11[pos_b] + D12[pos_b] + D13[pos_b] + D14[pos_b] +
                D15[pos_b] + D16[pos_b] + D17[pos_b] + D18[pos_b] + D19[pos_b];
          avr /= model_size;
        }
      }
      rand_T = lfsr113_Bits(&rand_n[jj * width + ii]) % T_upper;
      if (rand_T < ratio) {
        mi = choose_m(&rand_n[jj * width + ii], model_size, M0, M1, M2, M3, M4,
