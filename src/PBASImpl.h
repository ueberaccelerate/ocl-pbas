...
private:
    ...
    */
    ...
    void set_arg_fill_R_T_kernel(cl_kernel &m_cl_fill_R_T_kernel, cl_mem &mem_T,
                                 const cl_uint &width, const cl_uint &height,
                                 cl_mem &mem_R, cl_int T, cl_int R);
    void set_arg_magnitude_kernel(cl_kernel &m_cl_magnitude_kernel, cl_mem &src,
                                  cl_uint width, cl_uint height, cl_mem &mag);
    void set_arg_average_Im(cl_mem &mem_Im, cl_uint width, cl_uint height,
                            cl_mem &mem_avrg_Im);
    void set_arg_pbas_part1(cl_mem &mem_feature, const int &width,
                            const int &height, cl_mem &mem_R, cl_mem &mem_D,
                            cl_mem &mem_M, cl_mem &mem_index_r,
                            cl_float average_mag);
    void set_arg_pbas_part2(cl_mem &mem_feature, const int width,
                            const int height, cl_mem &mem_R, cl_mem &mem_T,
                            cl_mem &mem_index_r, cl_uint min_v, int &cl_index,
                            const int model_size, cl_mem &mem_mask,
                            cl_mem &mem_avrg_d, cl_mem &mem_rand);
    void set_arg_update_R_T(cl_mem &mem_mask, const int width, const int height,
                            cl_mem &mem_R, cl_mem &mem_T, cl_mem &mem_avrg_d);
};
};
